...
class PlgCsviextJoomlacustomfields extends RantaiPluginDispatcher
{
	...
	public function importCustomfields($plugin, $fieldName, $value, $itemId, $log)
	{
		...
		$currentFieldOldValue = $this->db->loadResult();
		// END - Hack by Tekdi
		...
		switch (strtolower($fieldType))
		{
			case 'repeatable':
				...
				// START - Hack by Tekdi
				if (!empty($currentFieldOldValue) && $currentFieldOldValue !== null)
				{
					$currentFieldOldValue = json_decode($currentFieldOldValue, true);

					if (is_array($currentFieldOldValue) && count($currentFieldOldValue))
					{
						// Replace values from new array into old recursively
						$finalArray = array_replace_recursive($currentFieldOldValue, $finalArray);
					}
				}
				// END - Hack by Tekdi
